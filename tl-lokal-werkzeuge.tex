...
\section{busybox}
\label{sec:lokal-werkzeuge-busybox}
\begin{normaltext}
  ...
  Programm. Es ist mit Blick auf Größenoptimierung und geringe Ressourcen
  geschrieben. Für ein arbeitsfähiges System benötigt man nur noch ein /dev
  Verzeichnis mit den Gerätedateien, ein /etc Verzeichnis mit den
  Konfigurationsdateien und einen Linux-Kernel. Busybox ist extrem
  anpassungsfähig, beim Kompilieren kann ich festlegen, welche Befehle mit
  hineinkommen und so die Funktionalität mit der Programmgröße abwägen. Damit
  ist es ideal für eingebettete Systeme und andere kleine Systemumgebungen.

  Und damit bin ich wieder beim Punkt, warum man Busybox kennen sollte. Es
  befindet sich auf dem InitRamFS, dem ersten Dateisystem, das zusammen mit
  dem Kernel vom Bootlader geladen wird. Auf Live-CDs, beim Netboot und
  bei Rescue-Disks kann ich mit Busybox rechnen. Auch in kleineren
  Einzelgeräten, wie Routern, wird es gern eingesetzt.

  Busybox ist ein so genanntes Multi-Call-Binary. Das heisst, es verhält sich
  komplett anders, je nachdem, wie es aufgerufen wird. Sämtliche UNIX-Befehle,
  die beim Kompilieren aktiviert wurden, kann ich auf zwei Arten aufrufen.
...
$ ./cp file file.bak
  ...
  Funktionen mein konkretes Binary kennt. Dazu rufe ich Busybox einfach ohne
  Argumente mit seinem eigenen Namen auf:
  \begin{verbatim}
$ busybox
...
Currently defined functions:
  ...
  täglich benutzten in Details abweichen, muss ich mitunter die genauen
  Optionen wissen. Neben der Handbuchseite, die ich in einer eingeschränkten
  Umgebung oft genug nicht zur Verfügung habe, hilft mir auch hier wieder
  Busybox selbst, indem ich den Befehl mit der Option --help aufrufe, die von
  den meisten der implementierten Befehle verstanden wird:
  \begin{verbatim}
$ busybox ln --help
BusyBox v1.18.5 (Ubuntu 1:1.18.5-1ubuntu4.1) multi-call binary.
...
\begin{normaltext}
  ...
  \cite{ChristiansenTorkington04de}. In diesem sind Lösungen
...
05:my $wanted = 'CRON';
06:my $syslogline = qr/^
07:    (\S{3})\s{1,2}              # $1: month name
08:    (\d{1,2})\s                 # $2: day of month
09:    (\d\d):(\d\d):(\d\d)\s      # $3, $4, $5: hour, minute, second
10:    (\S+)\s                     # $6: host
11:    ([^[:]+)                    # $7: process name
12:    (?:\[(\d+)\])?              # $8: process id
13:    :\s
14:    (.+)                        # $9: log line
15:    $/x;
16:my ($sec,$min,$hour,$mday,$month,$year,$wday,$yday,$isdst) = localtime time;
18:while (<>) {
19:    if (/$syslogline/) {
20:        my $time = utctime($1, $2, $3, $4, $5);
21:        if ($7 eq $wanted) {
22:            process_line($time,$6,$7,$8,$9);
23:        }
24:    }
25:}
27:sub utctime {
28:    my ($month,$dom,$hour,$min,$sec) = @_;
29:    my %months = (
30:        Jan => 0, Feb => 1, Mar => 2, Apr => 3, May => 4, Jun => 5,
31:        Jul => 6, Aug => 7, Sep => 8, Oct => 9, Nov => 10, Dec => 11,
32:    );
33:    return timegm($sec, $min, $hour, $dom, $months{$month}, $year);
34:}
36:sub process_line {
37:    print join(" ",@_,"\n");
38:    # ...
39:}
  \end{verbatim}
  In Zeile 05 gebe ich den Prozess an, an dessen Logzeilen ich interessiert
  bin. Sind es mehrere Prozesse, muss ich gegebenenfalls die Zeile 21
  anpassen.

  Die Zeilen 06-15 definieren einen regulären Ausdruck mit dessen Hilfe ich
  die Protokollzeilen aufspalte in die angegebenen Bestandteile.

  Die Zeitfelder fasse ich in der Funktion \verb?utctime()? zu einer Zahl
  zusammen. Als Jahr nimmt das Skript das aktuelle an, dieses wird in zeile 16
  ermittelt.

  In Funktion \verb?process_line()? ab Zeile 36 kann ich die gesamte
